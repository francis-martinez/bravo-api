...
fn get_iam_public_api_key() -> Result<String, JsValue> {
    std::env::var("VITE_IAM_PUBLIC_API_KEY")
        .map_err(|e| JsValue::from_str(&format!("Environment variable error: {}", e)))
}
...
pub async fn login(login_id: String, password: String) -> Result<JsValue, JsValue> {
    ...
    let response = client
        ...
        .header(reqwest::header::AUTHORIZATION, get_iam_public_api_key()?)
        .json(&body)
        .send()
        .await
